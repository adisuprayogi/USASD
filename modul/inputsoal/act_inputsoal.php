...
$menu = 'inputsoal';
$user_id = isset($_POST['user_id']) ? htmlspecialchars($_POST['user_id'],ENT_QUOTES) : '';
...
case 'simpan':
	...
	if(empty($mkkode) || empty($judul) || empty($bobot) || empty($soal) || empty($sa) || empty($ja) || empty($sb) || empty($jb) || empty($sc) || empty($jc) || empty($sd) || empty($jd)){
		echo "<script>alert('Data belum diisi dengan lengkap');history.back();</script>";
	}
	else{
		$insert=mysql_query("INSERT INTO soal(mkkode,judul,bobot,soal,ja,sa,jb,sb,jc,sc,jd,sd,status,tgl_insert,tgl_edit,utsuas,inp,upd) VALUES('$mkkode','$judul','$bobot','$soal','$ja','$sa','$jb','$sb','$jc','$sc','$jd','$sd','A',NOW(),NOW(),'A','$user_id','$user_id')");
		if($insert){
			$status = '11';
		}
		else{
...
case 'update':
	...
	if(empty($mkkode) || empty($judul) || empty($bobot) || empty($soal) || empty($sa) || empty($ja) || empty($sb) || empty($jb) || empty($sc) || empty($jc) || empty($sd) || empty($jd)){
		...
		$update=mysql_query("UPDATE soal set judul='$judul',bobot='$bobot',soal='$soal',ja='$ja',sa='$sa',jb='$jb',sb='$sb',jc='$jc',sc='$sc',jd='$jd',sd='$sd',upd='$user_id',tgl_edit=NOW() where id_soal='$ids'");
		if($update){
			$status = '21';
		}
		else{
