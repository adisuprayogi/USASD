...
(SELECT * FROM master_ujian WHERE mhswid='$mhsid' AND STATUS ='A' and utsuas='T')a LEFT JOIN
(SELECT * FROM jadwal_ujian WHERE utsuas='T' AND STATUS in ('N','F'))b ON a.id_jujian=b.id_jujian LEFT JOIN
(SELECT * FROM mk WHERE na='N')c ON b.mkkode=c.mkkode GROUP BY b.mkkode order by b.tgl_ujian,b.jam_mulai");
	...
	$tgls=format_tgl(($d['tgl_ujian']),'dd-mmmm-yyyy');
		...
		<tr class='data'>
			...
			if (($d['st'])=="A"){
			echo"
			<a href='index.php?m=kerjasoaluts&id=$d[id_ujian]&idj=$d[id_jujian]&ur=1' title='Start Exam'><img src='css/img/start.png'></a>";
			}else{
				echo"
			<a href='index.php?m=$menu' title='Tidak Aktif'><img src='css/img/tdk.png'></a>";
			}
			echo"
			</center>
			</td>
